...
#define SERVO_FREQ 333 // Analog servos run at ~50 Hz updates, digital at ~333 Hz
...
void StackKame::execute(int steps, int period[8], int amplitude[8], int offset[8], int phase[8])
{
    ...
    for (int i = 1; i < 8; i++) {
        if (period[i] > maxPeriod) maxPeriod = period[i];
    }
...
void StackKame::walk(int steps, int period, bool reverse)
{
    ...
    if (!reverse) {
        ...
        for (int i = 0; i < 8; i++) phase[i] = tempPhase[i];
    } else {
    ...
    int periods[8];
    for (int i = 0; i < 8; i++) periods[i] = period;
